...
namespace ScalableMCP.Editor
{
    ...
    public class TestRunnerService : ICallbacks
    {
        ...
        // Default timeout in seconds for test runs
        ...
        public async Task<JObject> ExecuteTestsAsync(TestMode testMode, bool returnOnlyFailures, bool returnWithLogs, string testFilter = "")
        {
            ...
            return await WaitForCompletionAsync(DefaultTimeoutSeconds);
        }
        ...
        public void RunStarted(ITestAdaptor testsToRun)
        {
            if (_tcs == null) return;
            _results.Clear();
            Debug.Log($"[ScalableMCP] Test run started: {testsToRun?.Name}");
        }
        ...
        private async Task<JObject> WaitForCompletionAsync(int timeoutSeconds)
        {
            ...
            if (winner != _tcs.Task)
            {
                _tcs.TrySetResult(JsonResponseFactory.Error(
